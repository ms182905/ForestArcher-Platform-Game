...
class PlayerInfo:
    ...
    def draw(self, health):
        ...
        img = pygame.transform.chop(self.heart_black,
                                    (0, self.heart_black.get_height() * ratio, 0, self.heart_black.get_height()))
...
def generate_world():
    ...
    for template_number in range(COLS // 10 - 2):
        ...
        with open(map_templates[rng], newline='') as csvfile:
            ...
            for count, row in enumerate(reader):
                for y, tile in enumerate(row):
                    world_data[count][(template_number + 1) * 10 + y] = int(tile)

    with open('map_templates/map_template_end.csv', newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=';')
        for count, row in enumerate(reader):
            for y, tile in enumerate(row):
                world_data[count][COLS - 10 + y] = int(tile)
